...
public class Logger {
    private static final File defaultFile = new File(ServerPlus.getInstance().getDataFolder().getPath() + "\\Logs\\latest.log");
    ...
    public void createDefaultLog() {
        File folder = new File(ServerPlus.getInstance().getDataFolder().getPath() + "\\Logs");
        // Check log folder.
        if (!folder.exists()) {
            try {
                if (!folder.mkdir()) {
    ...
    public void createCustomLog() {
        ...
        if (path.startsWith("plugin//")) {
            path = path.replace("plugin/", ServerPlus.getInstance().getDataFolder().getPath());
